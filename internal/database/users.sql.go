...
VALUES ($1, $2, $3, NOW(), NOW()) RETURNING id, email, created_at, updated_at, hashed_password, is_chirpy_red
`
...
func (q *Queries) UpdateChirpyRedById(ctx context.Context, arg UpdateChirpyRedByIdParams) (User, error) {
	row := q.db.QueryRowContext(ctx, updateChirpyRedById, arg.ID, arg.IsChirpyRed)
	var i User
	err := row.Scan(
		&i.ID,
		&i.Email,
